/*
  # Call Monitoring Database Schema - Fixed Migration
...
SELECT 
  ...
  (
    ...
    FROM (
      SELECT unnest(kd2.keywords) as keyword
      FROM keyword_detections kd2
      WHERE kd2.call_id = c.id
      AND kd2.keywords IS NOT NULL
    ) subq
    WHERE keyword IS NOT NULL
  ) as all_keywords_detected
FROM calls c
